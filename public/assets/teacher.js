...
function renderStudents() {
  ...
  panel.innerHTML = `
    ...
    <article class="card card-pad">
      <div class="section-heading">
        <div><h2>学生账号</h2><p>可以改用户名、重置密码、停用账号或强制退出。</p></div>
        <span class="chip">${users.length} 个账号</span>
      </div>
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>用户名</th><th>教师备注</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
            ${
              users.length
                ? users
                    ...
                    .join("")
                : '<tr><td colspan="4" class="empty-state">还没有学生账号。</td></tr>'
            }
          </tbody>
        </table>
      </div>
              ...
              attempts.length
                ? attempts
                    .map(
                      (attempt) => `
                        <tr>
                          <td><input type="checkbox" data-attempt-check value="${attempt.id}" ${attempt.status !== "draft" ? "disabled" : ""} /></td>
                          ...
                          <td><div class="table-actions">
                            <button class="button button-primary button-small" data-review="${attempt.id}">${attempt.status === "draft" ? "锁定并批改" : "打开批改"}</button>
                    ...
                    .join("")
                : '<tr><td colspan="6" class="empty-state">这套练习还没有学生保存答案。</td></tr>'
            }
          </tbody>
        </table>
      </div>
    ...
    <article class="card card-pad" style="margin-top:20px">
      ...
      <div class="review-list">
        ${items
          .map(
            (item) => `
              <section class="review-item" data-grade-item="${escapeHtml(item.question_id)}">
                <div>
